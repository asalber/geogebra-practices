...
% and la integración son operaciones inversas.
...
% conjunto de todas sus funciones primitivas and se representa como:

% \[
% \ \int{f(x)}\,dx=F(x)+C
% \]
% siendo $F(x)$ una función primitiva de $f(x)$ and $C$ una constante arbitraria.\\


% \subsubsection*{Linealidad de la integral}

% Dadas dos funciones $f(x)$ and $g(x)$ que admiten primitiva, and una
...
% Dada una función $f:[a,b]\rightarrow\mathbb{R}$ acotada and una
% partición $P=\{x_{0},x_{1},...,x_{n}\}$ de $[a,b]$, se llama
% \emph{suma inferior} de $f$ en relación a $P$, and se designa por
% $L(P,f)$, a:
...
% La \emph{suma inferior} and la \emph{suma superior} así definidas
% representan las sumas de las áreas de los rectángulos que tienen por
% bases los subintervalos de la partición, and por alturas los valores
% mínimo and máximo respectivamente de la función $f$ en los
% subintervalos considerados, tal and como se muestra en la
% figure~\ref{g:sumassupinf}. Así, los valores de $L(P,f)$ and $U(P,f)$
% serán siempre menores and mayores respectivamente, que el área
% encerrada por la función $f$ and el eje de abscisas en el intervalo
% $[a,b]$.
...
% \]
% and ese número se designa por $\int_{a}^{b}f(x)\,dx$ o simplemente por
% $\int_{a}^{b}f$.
...
% Dadas dos funciones $f$ and $g$ integrables en $[a,b]$ and $k \in
...
% Dadas dos funciones $f$ and $g$ integrables en $[a,b]$ and tales que
...
% Si $f$ es una función acotada en $[a,b]$ and $c\in(a,b)$, entonces $f$
% es integrable en $[a,b]$ si and sólo si lo es en $[a,c]$ and en $[c,b]$,
...
% Sea $f : [a,b]\rightarrow\mathbb{R}$ continua and sea $G$ una función
% continua en $[a,b]$. Entonces $G$ es derivable en $(a,b)$ y
% $G'(x)=f(x)$ para todo $x\in(a,b)$ si and sólo si:
...
% Si $f$ es una función continua en $[a,b]$ and $G$ es continua en
% $[a,b]$, derivable en $(a,b)$ and tal que $G'(x)=f(x)$ para todo
% $x\in(a,b)$ entonces:
...
% \item El intervalo $(a,b)$ puede ser infinito and la definición sería análoga, pero en ese caso es preciso que la integral impropia sea convergente.

% \item Si $f(x)\geq0$ and $g(x)=0$ al calcular la integral entre $a$ and $b$ se obtiene el área encerrada por la función $f(x)$ and el eje de abscisas entre las rectas verticales $x=a$ and $x=b$ (figure~\ref{g:integral_definida}).

% \begin{figure}[h!]
% \begin{center}
% \scalebox{1}{\input{img/integrales/integral_definida}}
% \caption{Cálculo de área encerrada por la función $f(x)$ and el eje de
% abscisas entre las rectas verticales $x=a$ and $x=b$  mediante la
% integral definida.} \label{g:integral_definida}
% \end{center}
% \end{figure}

% \item Si $f(x)\geq 0$ $\forall x\in[a,c]$ and $f(x)\leq 0$ $\forall x\in[c,b]$, el área de la región plana encerrada por $f$, las rectas verticales $x=a$ and $x=b$ and el eje de abscisas se calcula
% mediante:
% \[
% \ A= \int_{a}^{c}{f(x)\,dx} - \int_{c}^{b}{f(x)\,dx}.
% \]

% \item Si las curvas $y=f(x)$ e $y=g(x)$ se cortan en los puntos de abscisas $a$ and $b$, no cortándose en ningún otro punto cuya abscisa esté comprendida entre $a$ and $b$, el área encerrada por dichas curvas entre esos puntos de corte puede calcularse
% mediante:
% \[
% \ A= \int_{a}^{b}{|f(x)-g(x)|dx}
% \]
...
% tienen áreas $A(x)=\pi(f(x))^{2}$, and cuyo volumen comprendido entre
% las abscisas $a$ and $b$ será:
...
% $y=f(x)$, $y=g(x)$ and las rectas verticales $x=a$ and $x=b$ es:
...
\item Compute las siguientes integrales definidas and representarlas gráficamente:
      ...
      \item $\displaystyle \int_{-\frac{1}{2}}^0 \frac{x^{3}}{x^{2}+x+1}\,dx$
            ...
            \item Enter the command \command{Integral(f(x), -1/2, 0)} and hacer clic en el botón de \button{Valor numérico}.
      ...
      \item $\displaystyle \int_2^4 \frac{\sqrt{16-x^{2}}}{x}\,dx$
            ...
            \item Enter the command \command{Integral(g(x), 2, 4)} and hacer clic en el botón de \button{Valor numérico}.
            \item Para representar gráficamente la región que abarca la integral definida, hacer clic en el círculo que aparece a la izquierda de la expresión anterior.
            \end{enumerate}
            \end{indication}
      ...
      \item $\displaystyle \int_0^{\frac{\pi}{2}} \frac{dx}{3+\cos(2x)}$
            ...
            \item Enter the command \command{Integral(h(x), 0, pi/2)} and hacer clic en el botón de \button{Valor numérico}.
            \item Para representar gráficamente la región que abarca la integral definida, hacer clic en el círculo que aparece a la izquierda de la expresión anterior.
            \end{enumerate}
            \end{indication}
      \end{enumerate}
...
\item Compute la siguiente integral impropia $\int_2^{\infty} x^2e^{-x}\,dx$ and representarla gráficamente.
      ...
      \item Enter the command \command{Integral(f(x), 2, inf)} and hacer clic en el botón de \button{Valor numérico}.
...
\item Representar la parábola $y=x^{2}-7x+6$, and calcular el área limitada por dicha parábola, el eje de abscisas and las rectas $x=2$ and $x=7$.
      ...
      \item Como $f$ toma valores positivos and negativos en el intervalo $(2,7)$, para obtener el área comprendida entre la $f$ and el eje de abscisas en este intervalo hay que calcular la integral del valor absoluto de $f$.
            Para ello Enter the command \command{Integral(abs(f(x)), 2, 7)} and hacer clic en el botón de \button{Valor numérico}.
      \item Para representar gráficamente la región que abarca la integral impropia, hacer clic en el círculo que aparece a la izquierda de la expresión anterior.
      \end{enumerate}
      \end{indication}
...
\item Compute and dibujar el área comprendida entres las funciones $\sin x$ and $\cos x$ en el intervalo $[0,2\pi]$.
      \begin{indication}
      \begin{enumerate}
      \item Introducir la función \command{f(x):=sen(x)} in the \field{Input Bar} of the  \field{CAS View} and activate the \field{Graphics View}.
      \item Introducir la función \command{g(x):=cos(x)} in the \field{Input Bar}.
      \item Enter the command \command{Integral(abs(f(x)-g(x)), 0, 2pi)} in the \field{Input Bar} and hacer clic en el botón de \button{Valor numérico}.
...
\item Representar gráficamente la región del primer cuadrante limitada por la función $f(x)=(x+\sin x)^2$, la recta $x=2\pi$ and el eje $OX$, and hallar el volumen generado en la rotación alrededor del eje $OX$ de la región anterior.
      \begin{indication}
      \begin{enumerate}
      \item Introducir la función \command{f(x):=(x+sen(x))\^{}2} in the \field{Input Bar} of the  \field{CAS View} and activate the \field{Graphics View}.
      \item Para calcular el área plana Enter the command \command{Integral(f(x), 0, 2pi)} in the \field{Input Bar} and hacer clic en el botón de \button{Valor numérico}.
      \item Para representar gráficamente el área, hacer clic en el círculo que aparece a la izquierda de la expresión anterior.
      \item Para calcular el volumen de revolución Enter the command \command{Integral(pif(x)\^{}2, x, 0, 2pi)}.
      \end{enumerate}
      \end{indication}
