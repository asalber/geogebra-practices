...
\item Given the function $f(x,y)=y^2-x^2$, se pide:
      ...
      \item Calcular la derivada de $f(1,y)$ en $y=2$.
            \begin{indication}
            \begin{enumerate}
            \item Introducir la expresión \command{Derivada(f(1,y))} in the \field{Input Bar}.
      ...
      \item Calcular la derivada de $f(x,2)$ en $x=1$.
            \begin{indication}
            \begin{enumerate}
            \item Introducir la expresión \command{Derivada(f(x,2))} in the \field{Input Bar}.
      ...
      \item Calcular la derivadas parciales de $f$ en el punto $(1,2)$. ¿Qué conclusiones sacas?
            \begin{indication}
            \begin{enumerate}
            \item To compute la derivada parcial con respecto a $x$, enter the command \command{f'\_x(x,y):=Derivada(f, x)} in the \field{Input Bar}.
            ...
            \item To compute la derivada parcial con respecto a $y$, enter the command \command{f'\_y(x,y):=Derivada(f, y)} in the \field{Input Bar}.
...
\item Given the function $f(x,y)=20-4x^2-y^2$, se pide calcular en el punto $(2,-3)$:
      ...
      \item Vector gradiente.
            ...
            \item To compute la derivada parcial con respecto a $x$, enter the command \command{f'\_x(x,y):=Derivada(f, x)} in the \field{Input Bar}.
            \item To compute la derivada parcial con respecto a $y$, enter the command \command{f'\_y(x,y):=Derivada(f, x)} in the \field{Input Bar}.
      ...
      \item Matriz hessiana.
            \begin{indication}
            \begin{enumerate}
            \item To compute la derivada parcial de segundo orden $\frac{\partial\^2 f}{\partial x^2}$, enter the command \command{f''\_\{xx\}(x,y):=Derivada(f'\_x, x)} in the \field{Input Bar}.
            \item To compute la derivada parcial de segundo orden $\frac{\partial\^2 f}{\partial y\partial x}$, enter the command \command{f''\_\{xy\}(x,y):=Derivada(f'\_x, y)} in the \field{Input Bar}.
            \item To compute la derivada parcial de segundo orden $\frac{\partial\^2 f}{\partial x\partial y}$, enter the command \command{f''\_\{yx\}(x,y):=Derivada(f'\_y, x)} in the \field{Input Bar}.
            \item To compute la derivada parcial de segundo orden $\frac{\partial\^2 f}{\partial y^2}$, enter the command \command{f''\_\{yy\}(x,y):=Derivada(f'\_y, y)} in the \field{Input Bar}.
...
\item Hallar la recta normal y el plano tangente a la superficie $S: x+2y-\log z +4 =0$ en el punto $(0,-2,1)$ y dibujarlos.
      ...
      \item To compute el gradiente, enter the expression \command{f'(x,y,z):=(Derivada(f,x), Derivada(f,y), Derivada(f,z))} in the \field{Input Bar}.
...
\item Calcular la derivada direccional de la función $h(x,y)= 3x^2+y$ en el punto $(0,0)$, en la dirección del vector $(1,1)$.
      ...
      \item To compute la derivada parcial con respecto a $x$, enter the command \command{h'\_x(x,y):= Derivada(h, x)} in the \field{Input Bar}.
      \item To compute la derivada parcial con respecto a $y$, enter the command \command{h'\_y(x,y):= Derivada(h, y)} in the \field{Input Bar}.
...
\item Given the function $f(x,y)=x^3+y^3-3xy$, se pide:
      ...
      \item Definirla y dibujar su gráfica. ¿Puedes identificar a simple vista sus extremos relativos?
            \begin{indication}
            Introducir la función \command{f(x,y):=x\^{}3+y\^{}3-3x*y} in the \field{Input Bar} of the \field{CAS View} y activar la \field{Vista Gráficas 3D}.
            \end{indication}
