...
% concepto de derivada parcial en funciones de varias variables and presentamos algunas de sus aplicaciones.
...
% $\dfrac{{df}} {{dx}}(a)$, and se calcula como:
...
% que se deriva, and constantes el resto. Como consecuencia, las reglas
...
% constante e igual a $y_0$, and tan sólo varía el valor de $x$. E
...
% a la curva descrita por la intersección de la superficie de $f$ and el
...
% límites que las definen, siempre and cuando existan, obtenemos las
% segundas, terceras and derivadas de cualquier orden. Es decir, si $f$
...
% parciales segundas, and así sucesivamente hasta órdenes superiores de
% derivación.
...
% \] and cuatro diferentes de segundo orden, que también serán funciones de $x$ e $y$, aunque ya no se
...
% La primera and la última reciben el nombre de derivadas segundas, mientras que la segunda and tercera se denominan derivadas
% cruzadas.

% Si procedemos con las derivadas parciales de tercer orden tendríamos ocho diferentes, and el número es más amplio con
...
% \] 
% e igual con las derivadas cruzadas de tercer and superior orden.
% \end{teorema}
...
% \subsection{Vector gradiente and matriz hessiana}
...
% Dado un campo escalar $f(x_1,\ldots,x_n)$, se llama \emph{gradiente} de $f$, and se escribe $\nabla f$, a la función que a
...
% El vector gradiente en un punto dado tiene la misma magnitud and dirección que la velocidad máxima de variación de la
...
% orden en un punto $a=(a_1,\ldots,a_n)$, se define la \emph{matriz hessiana} de $f$ en $a$, and se nota $\nabla^2f(a)$, como la
...
% \right).
% \]
% Al determinante de esta matriz se le llama \emph{hessiano} de $f$ en $a$, and se nota $Hf(a)=|\nabla^2f(a)|$.
% \end{definicion}

% Entre las utilidades de la matriz Hessiana and el hessiano está la determinación de los extremos relativos and los puntos de
...
% Dado un campo escalar $f$ de $\mathbb{R}^n$, un punto $a$ and un vector unitario $\mathbf{u}$ en ese espacio, el límite
% \[
...
% Existen puntos como el de la figure~\ref{g:punto_silla} donde se anula el gradiente and que no son puntos de máximo, ni de
% mínimo.
...
% matriz Hessiana and el hessiano. 
...
% \end{itemize}
% \end{teorema}
% En el caso de que $\nabla^2f(a)$ sea semidefinido, no se puede obtener ninguna conclusión and hay que recurrir a derivadas
...
% \begin{itemize}
% \item Si $Hf(a)>0$ and $\dfrac{\partial^2 f}{\partial x^2}(a)>0$ entonces $f$ tiene un mínimo relativo en $a$.
% \item Si $Hf(a)>0$ and $\dfrac{\partial^2 f}{\partial x^2}(a)<0$ entonces $f$ tiene un máximo relativo en $a$.
...
\item Compute la recta tangente and el plano normal a la trayectoria
      ...
      \begin{indication}
      \begin{enumerate}
      \item Enter the command \command{f(t):=(sen(t),cos(t),sqrt(t))} in the \field{Input Bar} of the  \field{CAS View} and activar a \field{3D Graphics View}.
      \item Para dibujar el punto de tangencia en la trayectoria, Enter the command \command{f(pi)} in the \field{Input Bar} and hacer clic en el círculo que aparece a la izquierda de esta expresión.
      \item Para calcular and dibujar la recta tangente a la trayectoria, Enter the command \command{g(t):=f(pi)+f'(pi)t} in the \field{Input Bar}.
      \item Para calcular el plano normal a la trayectoria, Enter the command \command{((x,y,z)-f(pi))f'(pi)=0} in the \field{Input Bar}.
...
\item Dada la función $f(x,y)=y^2-x^2$, se pide:
      ...
      \item Dibujar su gráfica and el punto $(1,2,f(1,2))$.
            \begin{indication}
            \begin{enumerate}
            \item Introducir el la función \command{f(x,y):=y\^{}2-x\^{}2} in the \field{Input Bar} of the  \field{CAS View} and activar la \field{3D Graphics View}.
            \item Introducir el punto \command{(1,2,f(1,2))} in the \field{Input Bar} and hacer clic en el círculo que aparece a la izquierda de esta expresión.
            \end{enumerate}
      ...
      \item Dibujar el plano $x=1$. ¿Qué figure forma la intersección de este plano con la gráfica de $f$?
            \begin{indication}
            Enter the command \command{x=1} in the \field{Input Bar} and hacer clic en el círculo que aparece a la izquierda de esta expresión.
            \end{indication}

      \item Compute la derivada de $f(1,y)$ en $y=2$.
            \begin{indication}
            \begin{enumerate}
            \item Enter the expression \command{Derivada(f(1,y))} in the \field{Input Bar}.
            \item Enter the expression \command{Sustituye(\$, y=2)} in the \field{Input Bar}.
            \end{enumerate}
            \end{indication}

      \item Dibujar el plano $y=2$. ¿Qué figure forma la intersección de este plano con la gráfica de $f$?
            \begin{indication}
            Enter the command \command{y=2} in the \field{Input Bar} and hacer clic en el círculo que aparece a la izquierda de esta expresión.
            \end{indication}

      \item Compute la derivada de $f(x,2)$ en $x=1$.
            \begin{indication}
            \begin{enumerate}
            \item Enter the expression \command{Derivada(f(x,2))} in the \field{Input Bar}.
            \item Enter the expression \command{Sustituye(\$, x=1)} in the \field{Input Bar}.
            \end{enumerate}
            \end{indication}

      \item Compute la derivadas parciales de $f$ en el punto $(1,2)$. ¿Qué conclusiones sacas?
            \begin{indication}
            \begin{enumerate}
            \item Para calcular la derivada parcial con respecto a $x$, Enter the command \command{f'\_x(x,y):=Derivada(f, x)} in the \field{Input Bar}.
            \item Para calcular el valor de la derivada parcial con respecto a $x$ en el punto $(1,2)$, introducir la función \command{f'\_x(1,2)} in the \field{Input Bar}.
            \item Para calcular la derivada parcial con respecto a $y$, Enter the command \command{f'\_y(x,y):=Derivada(f, y)} in the \field{Input Bar}.
            \item Para calcular el valor de la derivada parcial con respecto a $y$ en el punto $(1,2)$, introducir la función \command{f'\_y(1,2)} in the \field{Input Bar}.
            \end{enumerate}
            \end{indication}
      \end{enumerate}


\item Compute las siguientes derivadas parciales:
      \begin{enumerate}
      \item  $\dfrac{\partial }{\partial V}\dfrac{nRT}{V}.$
            \begin{indication}
            Enter the command \command{Derivada(nRT/V, V)} in the \field{Input Bar} of the  \field{CAS View}.
            \end{indication}

      \item  $\dfrac{\partial ^{2}}{\partial x\partial y}e^{x+y}\sin(x/y).$
            \begin{indication}
            Enter the command \command{Derivada(Derivada(e\^{}(x+y)sen(x/y), y), x)} in the \field{Input Bar}.
            \end{indication}
      \end{enumerate}
...
\item Dada la función $f(x,y)=20-4x^2-y^2$, se pide calcular en el punto $(2,-3)$:
      ...
      \item Vector gradiente.
            \begin{indication}
            \begin{enumerate}
            \item Introducir la función \command{f(x,y):=20-4x\^{}2-y\^{}2} in the \field{Input Bar} of the  \field{CAS View}.
            \item Para calcular la derivada parcial con respecto a $x$, Enter the command \command{f'\_x(x,y):=Derivada(f, x)} in the \field{Input Bar}.
            \item Para calcular la derivada parcial con respecto a $y$, Enter the command \command{f'\_y(x,y):=Derivada(f, x)} in the \field{Input Bar}.
            \item Para calcular el vector gradiente, Enter the command \command{f'(x,y):=(f'\_x, f'\_y)} in the \field{Input Bar}.
      ...
      \item Matriz hessiana.
            \begin{indication}
            \begin{enumerate}
            \item Para calcular la derivada parcial de segundo orden $\frac{\partial\^2 f}{\partial x^2}$, Enter the command \command{f''\_\{xx\}(x,y):=Derivada(f'\_x, x)} in the \field{Input Bar}.
            \item Para calcular la derivada parcial de segundo orden $\frac{\partial\^2 f}{\partial y\partial x}$, Enter the command \command{f''\_\{xy\}(x,y):=Derivada(f'\_x, y)} in the \field{Input Bar}.
            \item Para calcular la derivada parcial de segundo orden $\frac{\partial\^2 f}{\partial x\partial y}$, Enter the command \command{f''\_\{yx\}(x,y):=Derivada(f'\_y, x)} in the \field{Input Bar}.
            \item Para calcular la derivada parcial de segundo orden $\frac{\partial\^2 f}{\partial y^2}$, Enter the command \command{f''\_\{yy\}(x,y):=Derivada(f'\_y, y)} in the \field{Input Bar}.
            \item Para calcular la matriz Hessiana, Enter the command \command{f''(x,y):=\{\{f''\_\{xx\}, f''\_\{yx\}\}, \{f''\_\{xy\}, f''\_\{yy\}\}\}} in the \field{Input Bar}.
            \item Para calcular la matriz Hessiana en el punto $(2,-3)$, introducir la función \command{f''(2,-3)} in the \field{Input Bar}.
            \end{enumerate}
            \end{indication}

      \item Compute el hessiano en el punto $(2,-3)$.
            \begin{indication}
            Enter the expression \command{Determinante(\$)} in the \field{Input Bar}.
            \end{indication}
      \end{enumerate}
      ...
      % \item En el cuadro de diálogo que aparece seleccionar la variable $y$ and hacer clic en el botón \button{Simplificar}.
...
\item Hallar la recta normal and el plano tangente a la superficie $S: x+2y-\log z +4 =0$ en el punto $(0,-2,1)$ and dibujarlos.
      \begin{indication}
      \begin{enumerate}
      \item Introducir la función \command{f(x,y,z):=x+2y-log(z)+4} in the \field{Input Bar} of the  \field{CAS View}.
      \item Para dibujar la gráfica de la función, introducir la ecuación \command{f(x,y,z)=0} in the \field{Input Bar} and activar a \field{3D Graphics View}.
      \item Para dibujar el punto, introducir el punto \command{(0,-2,1)} in the \field{Input Bar} and hacer clic en el círculo que aparece a la izquierda de esta expresión.
      \item Para calcular el gradiente, Enter the expression \command{f'(x,y,z):=(Derivada(f,x), Derivada(f,y), Derivada(f,z))} in the \field{Input Bar}.
      \item Para dibujar la recta normal, Enter the expression \command{n(t):=(0,-2,1)+f'(0,-2,1)t} in the \field{Input Bar}.
      \item Para dibujar el plano tangente, Enter the expression \command{((x,y,z)-(0,-2,1))f'(0,-2,1)=0} in the \field{Input Bar} and hacer clic en el círculo que aparece a la izquierda de esta expresión.
      \end{enumerate}
      \end{indication}

      % \item La ecuación $x+y-2e^y+2=0$ define implícitamente una función $y=f(x)$ alrededor del punto $(0,0)$. Compute
      % $f'(0)$.
      % \begin{indication}
      % \begin{enumerate}
      % \item Enter the expression \command{x+y-2\$e\^{}y+2}.
      % \item Hacer clic sobre el botón \button{Hallar una derivada}.
      % \item En el cuadro de diálogo que aparece seleccionar la variable $x$ and hacer clic en el botón \button{Simplificar}. 
      % \item Seleccionar de nuevo la expresión inicial.
      % \item Hacer clic sobre el botón \button{Hallar una derivada}.
      % \item En el cuadro de diálogo que aparece seleccionar la variable $y$ and hacer clic en el botón \button{Simplificar}.
      % \item Enter the expression \command{-\$i/\$j} donde \command{\$i} es la etiqueta de la expresión de la derivada
      % parcial con respecto a $x$ and \command{\$j} es la etiqueta de la expresión de la derivada parcial con respecto a $y$.
      % \item Hacer clic sobre el botón \button{Substitute}.
      % \item En el cuadro de diálogo que aparece, seleccionar $x$ e introducir el valor $0$, seleccionar $y$ e
      % introducir el valor $0$, and hacer clic en el botón \button{Simplificar}.
      % \end{enumerate}
      % \end{indication}
...
\item Compute la derivada direccional de la función $h(x,y)= 3x^2+y$ en el punto $(0,0)$, en la dirección del vector $(1,1)$.
      \begin{indication}
      \begin{enumerate}
      \item Introducir la función \command{h(x,y):=3x\^{}2+y} in the \field{Input Bar} of the  \field{CAS View}.
      \item Para calcular la derivada parcial con respecto a $x$, Enter the command \command{h'\_x(x,y):= Derivada(h, x)} in the \field{Input Bar}.
      \item Para calcular la derivada parcial con respecto a $y$, Enter the command \command{h'\_y(x,y):= Derivada(h, y)} in the \field{Input Bar}.
      \item Para calcular el vector gradiente, Enter the command \command{h'(x,y):= (h'\_x, h'\_y}) in the \field{Input Bar}.
      \item Para calcular la derivada direccional en el punto $(0,0)$ en la dirección del vector $(1,1)$, Enter the command \command{h'(0,0)VectorUnitario((1,1))}.
      \end{enumerate}
      \end{indication}
...
\item Dada la función $f(x,y)=x^3+y^3-3xy$, se pide:
      ...
      \item Definirla and dibujar su gráfica. ¿Puedes identificar a simple vista sus extremos relativos?
            \begin{indication}
            Introducir la función \command{f(x,y):=x\^{}3+y\^{}3-3x*y} in the \field{Input Bar} of the  \field{CAS View} and activar la \field{3D Graphics View}.
            \end{indication}
      ...
      \item Determinar los extremos relativos and los puntos de silla de $f$.
            ...
            \item Para calcular la derivada parcial de segundo orden $\frac{\partial\^2 f}{\partial y^2}$, Enter the command \command{f''\_\{yy\}(x,y):=Derivada(f'\_y, y)} in the \field{Input Bar}.
            \item Para calcular la matriz Hessiana, Enter the command \command{f''(x,y):=\{\{f''\_\{xx\}, f''\_\{yx\}\}, \{f''\_\{xy\}, f''\_\{yy\}\}\}} in the \field{Input Bar}.
            \item Para calcular el Hessiano, Enter the command \command{H(x,y):=Determinante(f'')} in the \field{Input Bar}.
            \item Para calcular el Hessiano en el punto crítico $(0,0)$ Enter the command \command{H(0,0)} in the \field{Input Bar}. Como sale negativo hay un punto de silla en $(0,0)$.
            \item Para calcular el Hessiano en el punto crítico $(1,1)$ Enter the command \command{H(1,1)} in the \field{Input Bar}. Como sale positivo hay un extremo relativo en $(1,1)$.
